...
export async function generateSpriteSheet(req: Request, res: Response) {
  try {
    const { characterImageUrl, type = "walk" } = req.body;

    if (!characterImageUrl) {
      return res.status(400).json({ error: "Character image URL is required" });
    }
    ...
    if (!result) {
      return res.status(500).json({ error: "No sprite sheet generated" });
    }
    ...
    return res.status(500).json({ error: "Failed to generate sprite sheet" });
  }
}
