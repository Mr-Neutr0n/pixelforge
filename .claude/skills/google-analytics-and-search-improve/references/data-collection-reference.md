...
## 1b. When Service Account Fails: OAuth Desktop Client

Some Google Cloud projects have restrictions that prevent Service Accounts (SA) from accessing certain APIs or properties. The two most common failures:

| Error | Meaning | Fix |
|---|---|---|
| `403 User does not have sufficient permission for site` | SA lacks property access | Use OAuth with the owner's personal Google account instead |
| `403 Request had insufficient authentication scopes` | ADC/gcloud token lacks required scopes | Use a custom OAuth Desktop client |

### When to use OAuth instead of SA

- **GSC**: If the site owner never added the SA email to Search Console users
- **GA4**: If the GA4 property is in an organization that blocks SA access
- **General**: If `gcloud auth application-default login` was used but the default client doesn't have `analytics`/`webmasters` scopes
...
5. Name: anything (e.g., `analytics-desktop`)
6. Click **CREATE → DOWNLOAD JSON**
7. Move the downloaded file to: `$DATA_DIR/configs/client_secret.json`
8. Re-run the script — it will open a browser for you to sign in with the Google account that owns the GSC/GA4 properties

The first OAuth run caches the token in `$DATA_DIR/.oauth-cache.json`. Subsequent runs reuse the cached token without browser interaction until it expires.

### Disabling SA to force OAuth fallback

If you previously placed an SA key in `$DATA_DIR/configs/` and want to switch to OAuth:

```bash
# Rename the SA key so scripts skip it
mv "$DATA_DIR/configs/"*.json "$DATA_DIR/configs/service-account.json.bak"
```

Scripts will then fall through to cached OAuth → ADC → fresh OAuth.

---

## 1c. Credential Discovery Flow (4-Tier Fallback)

`utils.py` implements a 4-tier fallback for Google authentication:

```
1. Service Account JSON key (configs/*.json)
2. Cached OAuth token (.oauth-cache.json)
3. Application Default Credentials (ADC) — gcloud auth application-default login
4. Fresh interactive OAuth flow (requires configs/client_secret.json)
```

**Tier 1 (SA)** is fastest and fully automatic. Use it when the SA email is authorized in both GSC and GA4.

**Tier 2 (Cached OAuth)** is used on subsequent runs after a successful Tier 4 login. Fully automatic, no browser.

**Tier 3 (ADC)** works if you ran `gcloud auth application-default login --scopes=...` with the correct scopes. The fallback verifies the token's actual scopes via the `tokeninfo` endpoint — if scopes are missing, it falls through to Tier 4 instead of failing silently.

**Tier 4 (Fresh OAuth)** is the last resort. Opens a browser for interactive consent. Caches the result for Tier 2 on next run.

---

## 1d. The `_CREDENTIALS` Pattern for GA4 Scripts

**Critical**: The Google Analytics Data API client libraries (`BetaAnalyticsDataClient`, `AlphaAnalyticsDataClient`) will silently fall back to Application Default Credentials (ADC) if you don't explicitly pass `credentials=` to the constructor. This means even after calling `get_credentials()`, the client may use a *different* credential object with different (or missing) scopes.

**Correct pattern** (used in `ga4_query.py` and `ga4_funnel.py`):

```python
from utils import get_credentials

# Capture credentials BEFORE importing Google client libraries
_CREDENTIALS = get_credentials()

from google.analytics.data_v1beta import BetaAnalyticsDataClient

# Pass credentials explicitly to the constructor
client = BetaAnalyticsDataClient(credentials=_CREDENTIALS)
```

**What happens if you skip this**:
- `get_credentials()` returns OAuth/SA credentials with the right scopes
- `BetaAnalyticsDataClient()` without `credentials=` auto-detects ADC
- ADC may have different scopes (or no scopes)
- Result: `403 Request had insufficient authentication scopes` even though `get_credentials()` succeeded

GSC scripts (`gsc_query.py`) don't need this pattern because `googleapiclient.discovery.build()` already passes `credentials=` explicitly.
