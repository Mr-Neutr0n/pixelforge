...
Read `analytics.config.json` at the repository root before collecting data. It is the source of truth for this site's canonical URL, GA4 property, measurement ID, and Search Console property. Keep credentials out of that file.
...
**Mode A**: Check `$DATA_DIR/.env` for required config. If missing, guide user to fill in `SITE_URL`, `GSC_SITE_URL`, `GA4_PROPERTY_ID`. Auth preference order (via `utils.py`): Service Account in `$DATA_DIR/configs/` → shared **GWS/CC** OAuth (`gws auth export`) → skill OAuth cache → ADC (only if scopes already cover GSC/GA4). **Do not open a browser or re-run `gws auth login` / ADC login unless silent credential resolution fails.** When re-auth is required, add analytics/GSC scopes onto the existing GWS/CC grant (union), never replace with analytics-only scopes. Run collection scripts to save data to `$DATA_DIR/data/`.
...
### Phase 7: Automated Analysis Pipeline (Optional)

> **Purpose**: After data collection, run a single script to generate all charts and analysis reports automatically.

Instead of manually writing individual analysis scripts for each phase, you can write a master `analyze.py` script that:

1. **Reads all raw data** from `$DATA_DIR/data/` (GSC JSON, GA4 JSON, PSI JSON, audit JSON)
2. **Processes and aggregates** each dataset using code (not manual JSON reading)
3. **Generates charts** for each analysis phase, saving PNGs to `$DATA_DIR/charts/`
4. **Writes Markdown reports** for each phase (GSC analysis, GA4 analysis, site audit, SEO/GEO checklist)
5. **Produces a summary JSON** with key metrics and scores for the final report generator

**Why use this pattern**: When you've already collected all the raw data, a single script guarantees consistency across all outputs. Every chart, metric, and report paragraph is derived from the same code logic. This eliminates the risk of mismatched numbers between charts and text.

**Template structure for `analyze.py`**:

```python
#!/usr/bin/env python3
"""Master analysis pipeline.

Reads raw data from $DATA_DIR/data/ and produces:
- charts/*.png     (one chart per analysis dimension)
- analysis/*.md    (markdown reports per phase)
- analysis/analysis_summary.json  (structured metrics for final report)
"""
...
# Discover DATA_DIR
SKILL_NAME = "google-analytics-and-search-improve"
def find_data_dir():
    d = Path(__file__).resolve().parent
    while d != d.parent:
        candidate = d / ".skills-data" / SKILL_NAME
        if candidate.is_dir():
            return candidate
        d = d.parent
    return None
...
gsc_queries = json.load(open(DATA / "gsc_queries.json")) if (DATA / "gsc_queries.json").exists() else {}
ga4_traffic = json.load(open(DATA / "ga4_traffic.json")) if (DATA / "ga4_traffic.json").exists() else {}
# ... load other files

# Phase 2: GSC analysis + chart generation
# Phase 3: GA4 analysis + chart generation
# Phase 4: Site audit analysis + chart generation
# Phase 5b: SEO/GEO checklist aggregation
# Phase 6: Write analysis_summary.json
```

**Chart generation in the same script**: Each analysis section should call matplotlib (or similar) to generate charts and save them to `$DATA_DIR/charts/`. Embed these charts in the Markdown reports using relative paths like `![Chart](../charts/gsc_ctr.png)`.

**Report reuse in Phase 6**: The final improvement report should read `analysis_summary.json` and embed charts generated in earlier phases by the same script. This ensures all numbers in the final report match the charts exactly.

---

## Troubleshooting

When scripts fail, check [references/troubleshooting-guide.md](references/troubleshooting-guide.md) before debugging:

| Error Pattern | Likely Cause | Quick Fix |
|---|---|---|
| `403 User does not have sufficient permission` | SA lacks property access | Use OAuth with owner's account, or add SA to GSC/GA4 |
| `403 Request had insufficient authentication scopes` | GA4 client silently used ADC instead of explicit credentials | Pass `credentials=_CREDENTIALS` to client constructor |
| `404 Site not found` | Wrong GSC property format | Use `sc-domain:example.com` for domain properties |
| `400 Invalid property` | Wrong GA4 Property ID | Check GA4 Admin → Property Settings |
| `429 Quota exceeded` | PSI API not enabled | Enable PageSpeed Insights API in Cloud Console |
| `DefaultCredentialsError` / OAuth setup required | No usable GWS/CC or cached credentials | Run `gws auth login --scopes='…union of existing GWS scopes + analytics.readonly + webmasters.readonly…'` — never a narrow ADC re-login |
...
| [references/troubleshooting-guide.md](references/troubleshooting-guide.md) | Common errors: auth failures, API errors, empty data, scope mismatches, deployment gotchas, and their root-cause fixes |
